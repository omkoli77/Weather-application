...
const Home = ()=>{
    ...
    useEffect(()=>{
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=mumbai&appid=1cf442e3a43bf41aa61d93ad10825e0a&units=metric`)
        .then((res)=> res.json())
        .then((res)=> {
            if(res.cod == 404){
        ...
        <div className="Home flex bg-purple-200 h-dvh justify-center items-center">
            <div className="weatherApp text-center w-100 bg-[linear-gradient(54deg,#2f4680,#500ae4)] h-140 rounded-xl">
                ...
                <div className="card flex justify-center text-gray-200 flex-col items-center  mt-10 ">
                    {data.humidity>80? <FaCloudSunRain className="text-7xl text-gray-400"/>: data.temp>29? <IoSunnySharp className="text-7xl text-yellow-500"/>: <FaRegSnowflake className="text-7xl text-pink-500"/>}
